...
void WeighRecordListDialog::initWeighRecordListCtr() //初始化listCtr
{
	...
	CString str[26]={"班次","工号","机器号","芯片型号","芯片来源","芯片批次","刷胶前重量","刷胶前称重时间","银浆序列号","银浆解冻完成时间","银浆过期时间",
	"刷胶后称重重量","刷胶后称重时间","胶重","芯片尺寸","是否超标","刮刀序列号","钢网序列号","垫片序列号","刮刀寿命","钢网寿命","垫片寿命","异常原因","处理方案","备注","处理人"};
	for (int i = 0; i < 26; i++)
	{
...
void WeighRecordListDialog::OnButton1() //查询
{
	...
	try
	{
		...
		sql.Format("SELECT shift,bn,machine_code,wafer_device,wafer_source,wafer_lot,first_weight,first_weigh_time,ep_sn,ep_start_time,ep_exceed_time,second_weight,\
					create_time,second_weight-first_weight,wafer_size,is_over_weight,scraper_sn,steel_mesh_sn,shim_sn,scraper_life,steel_mesh_life,shim_life,expection_reason,handle_plan,handle_remark,deal_person from wbc20_second_weigh_record WHERE create_time BETWEEN '%s' AND '%s'",beginTime,endTime);
